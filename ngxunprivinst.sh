...
if [ -f /etc/redhat-release ]; then
    ...
    REPOURL=https://plus-pkgs.nginx.com/centos/$RELEASE/$ARCH/RPMS/
    DISTRO="RHEL/CentOS"
    SUFFIX="el"
elif [ -f /etc/os-release ] && fgrep SLES /etc/os-release; then
    RELEASE=`grep -Eo 'VERSION="[0-9]{2}' /etc/os-release | cut -d'"' -f2`
    REPOURL=https://plus-pkgs.nginx.com/sles/$RELEASE/$ARCH/RPMS/
    DISTRO="SLES"
    SUFFIX="sles"
elif [ -f /etc/os-release ] && fgrep -q -i amazon /etc/os-release; then
    RELEASE=`grep -Eo 'VERSION=".+"' /etc/os-release | cut -d'"' -f2`
    if [ "$RELEASE" = "2" ]; then
        REPOURL=https://plus-pkgs.nginx.com/amzn2/2/$ARCH/RPMS/
        SUFFIX="amzn2"
    else
        REPOURL=https://plus-pkgs.nginx.com/amzn/latest/$ARCH/RPMS/
        SUFFIX="amzn1"
        RELEASE="1"
    fi
    DISTRO="amzn"
    ...
    REPOURL=https://plus-pkgs.nginx.com/$DISTRO/pool/nginx-plus/n/
elif [ -x /sbin/apk ]; then
    RELEASE=`grep -Eo 'VERSION_ID=[0-9]\.[0-9]{1,2}' /etc/os-release | cut -d'=' -f2`
    REPOURL=https://plus-pkgs.nginx.com/alpine/v$RELEASE/main/$ARCH/
    DISTRO="alpine"
else
...
if [ "$ACTION" = 'fetch' ] || [ "$ACTION" = 'list' ]; then
    if [ ! -f $NGXCERT ] || [ ! -f $NGXKEY ]; then
        ...
        ldd $(which wget) | grep -q libgnutls || \
            ...
            if ! $WGET -O /dev/null --certificate=$NGXCERT --private-key=$NGXKEY https://plus-pkgs.nginx.com/ ; then
                echo "Cannot connect to plus-pkgs.nginx.com, please check certificate and key."
                exit 1
            fi
    fi
fi
...
list() {
    if [ "$DISTRO" = 'ubuntu' ] || [ "$DISTRO" = 'debian' ]; then
        REPOURL=https://plus-pkgs.nginx.com/$DISTRO/pool/nginx-plus/n/nginx-plus
    fi
